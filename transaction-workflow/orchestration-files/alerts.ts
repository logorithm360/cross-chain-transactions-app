...
export async function alertsScreen(session: WalletSession): Promise<void> {
  while (true) {
    ...
    const choice = await askSelect("What would you like to do?", [
      ...
      { title: "Pause / delete a rule",  value: "manage",  description: "Turn off or remove an alert rule" },
...
async function collectRuleParams(
  ...
  switch (ruleType) {
    ...
    case "TOKEN_FLAGGED_SUSPICIOUS": {
      ...
      return { check_interval_seconds: checkInterval, auto_detect_tokens: true }
    }
...
async function historyFlow(session: WalletSession): Promise<void> {
  ...
  try {
    ...
    if (history.length === 0) {
      infoBox("No alerts have fired yet.")
      await pressEnter()
      return
    }
...
async function manageRuleFlow(session: WalletSession): Promise<void> {
  ...
  const action = await askSelect("What would you like to do?", [
    { title: "Pause rule",  value: "pause"  },
    { title: "Delete rule", value: "delete" },
  ])
  if (!action) return

  if (action === "delete") {
    warnBox("Deletion is permanent. The rule cannot be recovered.")
  }

  const ok = await confirm(`${action === "pause" ? "Pause" : "Delete"} rule #${ruleId}?`, false)
  if (!ok) return
  ...
  if (hash) successBox(`Rule ${action}d. Tx: ${hash}`)
  await pressEnter()
}
